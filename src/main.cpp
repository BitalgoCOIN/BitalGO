...
#if defined(NDEBUG)
#error "BitalGO cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ualg) are considered zero fee (for relaying, mining and transaction creation)
...
bool RecalculateALGSupply(int nHeightStart)
{
	AssertLockHeld(cs_main);
	...
	uiInterface.ShowProgress(_("Recalculating ALG supply..."), 0);
	while (true) {
		if (pindex->nHeight % 1000 == 0) {
			LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
			int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainHeight - nHeightStart))));
			uiInterface.ShowProgress(_("Recalculating ALG supply..."), percent);
		}
...
{
    util::ThreadRename("bitalgo-scriptch");
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> algInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
                algInputs.push_back(stakeIn);
        }
        const bool hasALGInputs = !algInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasALGInputs) {
                    ...
                    for (const CTxIn& algIn : algInputs)
                        if(algIn.prevout == in.prevout)
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction& t : bl.vtx) {
                    ...
                    for (const CTxIn& in: t.vin) {
                        ...
                        if (hasALGInputs) {
                            for (const CTxIn& stakeIn : algInputs)
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // BitalGO: We use certain sporks during IBD, so check to see if they are
