...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\BitalGO
// Windows >= Vista: C:\Users\Username\AppData\Roaming\BitalGO
// Mac: ~/Library/Application Support/BitalGO
// Unix: ~/.bitalgo
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitalGO";
#else
    fs::path pathRet;
...
{
    fs::path pathConfigFile(GetArg("-conf", "bitalgo.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty bitalgo.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override bitalgo.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(GetArg("-pid", "bitalgod.pid"));
