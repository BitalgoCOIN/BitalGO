...
bool CAlgStake::InitFromTxIn(const CTxIn& txin)
{
...
bool CAlgStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CAlgStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CAlgStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CAlgStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CAlgStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CAlgStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
CDataStream CAlgStake::GetUniqueness() const
{
    //The unique identifier for a ALG stake is the outpoint
...
CBlockIndex* CAlgStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CAlgStake::ContextCheck(int nHeight, uint32_t nTime)
{
