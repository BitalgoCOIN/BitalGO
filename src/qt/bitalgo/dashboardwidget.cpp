...
#include "qt/bitalgo/dashboardwidget.h"
#include "qt/bitalgo/forms/ui_dashboardwidget.h"
#include "qt/bitalgo/sendconfirmdialog.h"
#include "qt/bitalgo/txrow.h"
...
DashboardWidget::DashboardWidget(BitalGOGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareAlg, "square-chart-alg");
    setCssProperty(ui->labelAlg, "text-chart-alg");
    ...
    setCssProperty(ui->labelAmountAlg, "text-stake-alg-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &BitalGOGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    if (walletModel && walletModel->getOptionsModel()) {
        ...
        connect(ui->pushImgEmpty, &QPushButton::clicked, window, &BitalGOGUI::openFAQ);
        connect(ui->btnHowTo, &QPushButton::clicked, window, &BitalGOGUI::openFAQ);
    ...
    // update the display unit, to not use the default ("ALG")
    updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            alg = (pair.first != 0) ? pair.first / 100000000 : 0;
            chartData->totalAlg += pair.first;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesAlg.append(alg);

        int max = std::max(alg, alg);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(255, 145, 0));
    ...
    set0->append(chartData->valuesAlg);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalAlg > 0) {
        setCssProperty(ui->labelAmountAlg, "text-stake-alg");
    } else {
        setCssProperty(ui->labelAmountAlg, "text-stake-alg-disable");
    }
    forceUpdateStyle({ui->labelAmountAlg});
    ui->labelAmountAlg->setText(GUIUtil::formatBalance(chartData->totalAlg, nDisplayUnit));
