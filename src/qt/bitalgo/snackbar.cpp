...
#include "qt/bitalgo/forms/ui_snackbar.h"
#include "qt/bitalgo/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(BitalGOGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &BitalGOGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
