...
#include "qt/bitalgo/forms/ui_send.h"
#include "qt/bitalgo/addnewcontactdialog.h"
#include "qt/bitalgo/qtutils.h"
#include "qt/bitalgo/sendchangeaddressdialog.h"
#include "qt/bitalgo/optionbutton.h"
#include "qt/bitalgo/sendconfirmdialog.h"
#include "qt/bitalgo/myaddressrow.h"
#include "qt/bitalgo/guitransactionsutils.h"
...
SendWidget::SendWidget(BitalGOGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-alg");
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        CTxDestination algAdd = DecodeDestination(address.toStdString());
        if (walletModel->isMine(algAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(algAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
