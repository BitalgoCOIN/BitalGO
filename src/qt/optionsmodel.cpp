...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::ALG);
    ...
    if (settings.contains("nAnonymizeBitalGOAmount") || reset)
        SoftSetArg("-anonymizebitalgoamount", settings.value("nAnonymizeBitalGOAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in bitalgo.cpp during shotdown to also remove the window positions
