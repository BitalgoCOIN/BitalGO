...
#include "qt/bitalgo/splash.h"
#include "qt/bitalgo/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("bitalgo-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in bitalgo.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in bitalgo.qrc)
...
/** Class encapsulating BitalGO Core startup and shutdown.
...
/** Main BitalGO application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (BitalGOGUI)
...
{
    window = new BitalGOGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &BitalGOGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &BitalGOGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(BitalGOGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(BitalGOGUI::DEFAULT_WALLET);
        ...
        // BitalGO: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &BitalGOGUI::handlePaymentRequest);
        connect(window, &BitalGOGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. BitalGO can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(bitalgo_locale);
    Q_INIT_RESOURCE(bitalgo);
    ...
    /// 6. Determine availability of data directory and parse bitalgo.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("BitalGO"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("BitalGO"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    if (!masternodeConfig.read(strErr)) {
        QMessageBox::critical(0, QObject::tr("BitalGO Core"),
    ...
    // bitalgo: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("BitalGO didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
