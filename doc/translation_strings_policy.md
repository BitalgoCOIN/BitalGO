...
This document provides guidelines for internationalization of the BitalGO Core software.
...
This adds `<numerusform>`s to the respective `.ts` file, which can be translated separately depending on the language. In English, this is simply:
    ...
    <message numerus="yes">
        <source>%n active connection(s) to BitalGO network</source>
        <translation>
            <numerusform>%n active connection to BitalGO network</numerusform>
            <numerusform>%n active connections to BitalGO network</numerusform>
        </translation>
    </message>
...
This can be checked by executing `make translate` in the `src` directory, then verifying that `bitalgo_en.ts` remains unchanged.
